...
CREATE OR REPLACE FUNCTION register_dimension(source_id$i integer
                                               ...
                                               , data_type_id$i integer
                                               ) RETURNS integer AS $$
DECLARE
    dimension_id$i integer;
BEGIN
    insert into md_dimensions
    (source_id, schema, table_name, description, source_column, key_column, name_column, parent_column, data_type_id)
    values
    (source_id$i, schema$c, table_name$c, description$c, source_column$c, key_column$c, name_column$c, parent_column$c, data_type_id$i)
